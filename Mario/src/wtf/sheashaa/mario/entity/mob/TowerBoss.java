...
public class TowerBoss extends Entity {
	...
	public void render(Graphics g) {
		if (bossState == BossState.IDLE || bossState == BossState.SPINNING) g.setColor(Color.BLUE);
		else if (bossState == BossState.RECOVERING) g.setColor(Color.GREEN);
		else g.setColor(Color.RED);
		g.fillRect(x, y, width, height);
		g.setColor(Color.WHITE);
		g.setFont(new Font("Arial", Font.PLAIN, 15));
		g.drawString(bossState.toString(), x, y);
	}
	...
	public void tick() {
		...
		if ((phaseTime >= 180 && bossState == BossState.IDLE) || (phaseTime >= 600 && bossState == BossState.SPINNING) || (phaseTime >= 500 && bossState == BossState.RUNNING) || (phaseTime >= 600 && bossState == BossState.JUMPING)) chooseState();
		
		if (phaseTime >= 120 && bossState == BossState.RECOVERING) {
...
//					y = t.getY() - t.height;
		...
		for (int i = 0; i < handler.entity.size(); i++) {
			...
			if (e.getId() == Id.player) {
				if (bossState == BossState.JUMPING) {
					if (jumping || falling) {
						if (e.getX() >= getX() - 4 && e.getX() <= getX() + 4) setVelX(0);
